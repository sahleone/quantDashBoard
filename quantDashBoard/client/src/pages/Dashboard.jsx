  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const accountParam = selectedAccountId ? `&accountId=${encodeURIComponent(selectedAccountId)}` : "";
        ...
        const [portfolioValueRes, performanceRes, riskRes, factorRes] =
          await Promise.allSettled([
            authenticatedGet(`/api/portfolio/value?range=YTD${accountParam}`),
            authenticatedGet(`/api/metrics/performance?range=1Y${accountParam}`),
            authenticatedGet(`/api/metrics/risk?range=1Y${accountParam}`),
            authenticatedGet(`/api/metrics/factors?model=FF3&range=1Y${accountParam}`),
          ]);
          ...
          >
            ...
            {accounts.map((account) => (
              <option key={account.accountId} value={account.accountId}>
                {account.accountName || account.accountId} {account.institutionName ? `(${account.institutionName})` : ""}
              </option>
            ))}
          </select>
        </div>
